...
try {
    ...
    foreach ($carrito as $item) {
        ...
        if (!$stmt) {
            responder_error('Error preparando consulta de producto: ' . $conn->error, 500);
        }
    ...
    if (abs($total_bd - $total_cliente) > 0.01) {
        responder_error('El total del carrito no coincide con la base de datos. Actualiza la venta.', 400, [
    ...
    $external_reference = 'POS_' .
        date('YmdHis') . '_' .
        ($_SESSION['usuario_id'] ?? '0') . '_' .
