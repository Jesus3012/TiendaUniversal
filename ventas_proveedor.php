...
<link rel="stylesheet" href="css/ventas_proveedor.css?v=<?= time() ?>">
                        ...
                        <div class="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center gap-3">
                            ...
                            <div class="d-flex flex-column flex-sm-row align-items-stretch align-items-sm-center gap-2">
                                ...
                                <div class="d-flex gap-2">
                                    <div class="dropdown">
                                        <button class="btn btn-excel dropdown-toggle" type="button" data-bs-toggle="dropdown" aria-expanded="false" style="background: #22c55e; border: none; color: white; border-radius: 40px; padding: 8px 18px; font-size: 0.75rem; font-weight: 600;">
                                    ...
                                    <div class="dropdown">
                                        <button class="btn btn-pdf dropdown-toggle" type="button" data-bs-toggle="dropdown" aria-expanded="false" style="background: #ef4444; border: none; color: white; border-radius: 40px; padding: 8px 18px; font-size: 0.75rem; font-weight: 600;">
                            ...
                            <div class="table-wrapper">
                                <table class="productos-table" id="tablaProductos">
                                    <thead>
                                        <tr>
                                            <th>Producto</th>
                                            <th class="col-stock">Stock Inicial<br><span style="font-size: 0.65rem;"><?= $fechaHoy ?></span></th>
                                            <th class="col-conteo">Stock Contado<br><span style="font-size: 0.65rem;"><?= $fechaHoy ?></span></th>
                                    ...
                                    <tbody>
                                        ...
                                        <tr data-id="<?= $p['id'] ?>"
                                            ...
                                            <td class="nombre-producto">
                                                ...
                                                <?php if ($esPagado): ?>
                                                    <span class="badge-pagado"><i class="fas fa-check-circle"></i> Pagado</span>
                                                <?php else: ?>
                                                    <span class="badge-concesion"><i class="fas fa-handshake"></i> Concesión</span>
                                                <?php endif; ?>
                                                <br><small class="text-muted">Reg: <?= $fechaRegistro ?></small>
                                            </td>
                                            <td class="text-center"><strong class="text-primary"><?= number_format($stockInicial) ?></strong></td>
                                            <td class="text-center">
                                                <input type="number" class="stock-conteo" value="<?= $stockInicial ?>" min="0" max="<?= $stockInicial ?>" data-original="<?= $stockInicial ?>" style="width: 90px;">
                                                ...
                                                <input type="hidden" name="es_pagado[<?= $p['id'] ?>]" value="<?= $esPagado ? '1' : '0' ?>">
                                            </td>
                                        ...
                                        <div class="product-card-header">
                                            <div>
                                                ...
                                                <div class="product-meta">
                                                    <span class="product-date"><i class="far fa-calendar-alt"></i> <?= $fechaRegistro ?></span>
                                                </div>
                                            </div>
                                            <?php if ($esPagado): ?>
                                                <span class="badge-pagado-mobile"><i class="fas fa-check-circle"></i> Pagado</span>
                                            <?php else: ?>
                                                <span class="badge-concesion-mobile"><i class="fas fa-handshake"></i> Concesión</span>
                                            <?php endif; ?>
                                        </div>
                                        ...
                                        <div class="product-card-body">
                                            <div class="stats-grid">
                                                <div class="stat-card">
                                                    ...
                                                    <div class="stat-value primary"><?= number_format($stockInicial) ?></div>
                                                </div>
                                                <div class="stat-card">
                                                    <div class="stat-label">Stock Contado</div>
                                                    <input type="number" class="stock-conteo-mobile" value="<?= $stockInicial ?>" min="0" max="<?= $stockInicial ?>" data-original="<?= $stockInicial ?>">
                                                    <input type="hidden" name="ventas[<?= $p['id'] ?>]" class="ventas-input-mobile" value="0">
                                                    <input type="hidden" name="stock_final[<?= $p['id'] ?>]" class="stock-final-input-mobile" value="<?= $stockInicial ?>">
                                                </div>
                                            </div>
                                            
                                            <button type="button" class="btn-toggle-details" onclick="toggleProductDetails(this)">
                                            ...
                                            <div class="details-expanded">
                                                <div class="detail-row">
                                                    ...
                                                    <span class="detail-value ventas-mobile">0</span>
                                                </div>
                                                <div class="detail-row">
                                                    <span class="detail-label">Stock Final</span>
                                                    <span class="detail-value stock-final-mobile"><?= number_format($stockInicial) ?></span>
                                                </div>
                                                <div class="detail-row">
                                                    <span class="detail-label">Venta $</span>
                                                    <span class="detail-value venta-monto-mobile success">$0.00</span>
                                                </div>
                                                <?php if (!$esPagado): ?>
                                                <div class="detail-row">
                                                    <span class="detail-label">Deuda $</span>
                                                    <span class="detail-value deuda-monto-mobile danger">$0.00</span>
                                                </div>
                                                <?php endif; ?>
                                                <div class="detail-row">
                                                    <span class="detail-label">Ganancia $</span>
                                                    <span class="detail-value ganancia-monto-mobile success">$0.00</span>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
...
function toggleProductDetails(btn) {
    ...
    if (details.classList.contains('show')) {
        ...
        btn.innerHTML = '<i class="fas fa-chevron-down"></i> Ver detalles';
    } else {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function actualizarVistaMovil() {
        ...
        cards.forEach(card => {
            ...
            if (tablaRow) {
                const ventas = tablaRow.querySelector('.ventasCalculadas').innerHTML;
                const stockFinal = tablaRow.querySelector('.stockFinal').innerHTML;
                const ventaMonto = tablaRow.querySelector('.ventaMonto').innerHTML;
                ...
                card.querySelector('.venta-monto-mobile').innerHTML = ventaMonto;
                if (card.querySelector('.deuda-monto-mobile')) {
                    card.querySelector('.deuda-monto-mobile').innerHTML = deudaMonto;
                }
    ...
    function handleMobileInput() {
        ...
        if (parseInt(this.value) !== parseInt(this.dataset.original)) {
            this.classList.add('modificado');
            hayModificaciones = true;
        } else {
            this.classList.remove('modificado');
            hayModificaciones = Array.from(document.querySelectorAll('.stock-conteo, .stock-conteo-mobile')).some(inp => 
    ...
    function buscarProductos() {
        ...
        if (termino.length > 0) {
            ...
            if (contadorVisibles === 0) {
                document.querySelector('#tablaProductos tbody').appendChild(crearFilaNoResultados(termino));
            } else {
                eliminarFilaNoResultados();
            }
        } else {
    ...
    let chartVentas = new Chart(document.getElementById('graficaVentas'), {
        ...
        options: { responsive: true, maintainAspectRatio: true, plugins: { legend: { display: false } }, scales: { y: { beginAtZero: true, ticks: { callback: function(v) { return '$' + v.toLocaleString(); } } } } }
    });
    ...
    let chartStock = new Chart(document.getElementById('graficaStock'), {
        type: 'bar',
        data: { labels: nombres, datasets: [{ label: 'Stock Inicial', data: stockInicialArr, backgroundColor: '#3b82f6', borderRadius: 8 }, { label: 'Stock Final', data: stockFinalArr, backgroundColor: '#22c55e', borderRadius: 8 }] },
...
document.getElementById('btnGenerarPDF')?.addEventListener('click', function() {
    ...
    if (!fechaInicio || !fechaFin) {
        Swal.fire({ icon: 'warning', title: 'Fechas requeridas', text: 'Por favor selecciona ambas fechas', confirmButtonColor: '#ef4444' });
        return;
    }
    ...
    setTimeout(() => {
        Swal.close();
        Swal.fire({ icon: 'success', title: 'PDF Generado', text: 'El PDF se ha descargado', timer: 2000, showConfirmButton: false });
    }, 1500);
});
</script>
